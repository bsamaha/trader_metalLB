...
sed "s/\$NAMESPACE/$NAMESPACE/g" k3s-configs/influxdb/influx-deployment.yaml | kubectl apply -f -
...
INFLUXDB_IP=$(kubectl get svc -n $NAMESPACE influxdb-lb -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null)
INFLUXDB_PORT=$(kubectl get svc -n $NAMESPACE influxdb-lb -o jsonpath='{.spec.ports[0].port}' 2>/dev/null)
CHRONOGRAF_IP=$(kubectl get svc -n $NAMESPACE chronograf -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null)
CHRONOGRAF_PORT=$(kubectl get svc -n $NAMESPACE chronograf -o jsonpath='{.spec.ports[0].port}' 2>/dev/null)
